...
**\*I made a copy of the testing data and training data, I will delete the copy at the end of the analysis**

#### Drop education column in train and test

```{r}
#make a copy of the train.data.2 and test.data
train.data.2_copy <- subset(train.data, select = -c(education))
test.data.2_copy <- subset(test.data, select = -c(education))
```

#### Convert income column to numeric

```{r}
train.data.2_copy$income <- as.numeric(ifelse(train.data.2_copy$income == ">50K","1","0"))
test.data.2_copy$income <- as.numeric(ifelse(test.data.2_copy$income == ">50K","1","0"))
```
...
numeric_predictors <- train.data.2_copy[, sapply(train.data.2_copy, is.numeric) & names(train.data.2_copy) != "income"]
categorical_data <- train.data.2_copy[sapply(train.data.2_copy, is.factor)]
#store income
income <- train.data.2_copy$income  # Keep income unscaled
...
train.data.2_copy <- cbind(scaled_numeric, categorical_data, income)
head(train.data.2_copy)
```
...
# for test data
# Separate numeric predictors (exclude income) and categorical columns
numeric_predictors <- test.data.2_copy[, sapply(test.data.2_copy, is.numeric) & names(test.data.2_copy) != "income"]
categorical_data <- test.data.2_copy[sapply(test.data.2_copy, is.factor)]
...
# Combine scaled numeric data with categorical columns
test.data.2_copy <- cbind(scaled_test_numeric, categorical_data, income = test.data.2_copy$income)
head(test.data.2_copy)
```
...
```{r}
#fit mlr
mlr_model <- lm(income ~ ., data = train.data.2_copy)
```
...
```{r}
#prediction
pred_mlr <- predict(mlr_model, test.data.2_copy)
```
