...
# Therefore, we keep the original 'education' factor variable and remove 'education.num'.
...
train.data.2 <- dummy_cols(
  train.data.2,
  select_columns = c("workclass", "marital.status", "occupation", "relationship", 
...
### NOTE: ### The level "Holand-Netherlands" appeared in the training set but 
...
### NOTE: ### Check if the column names of train and test datasets are identical.
# Model prediction requires the exact same column order, so we align test data 
# columns to match the training set.
...
change target income column to 0/1
data scaling
