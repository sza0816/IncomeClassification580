...
``` {r}
library(NeuralNetTools) # Zian can u pls add this to the libraries. I added it here so u dont accidentally miss it on the combined code

# Reduced set of features (remove race, sex, native.country)
train.nn.reduced <- subset(train.data.2, select = -c(race_Black, race_White, sex_Male, `native.country_United-States`))
...
#     workclass_Local.gov + workclass_Private + workclass_Self.emp.not.inc +
#     marital.status_Married.civ.spouse + marital.status_Never.married +
#     occupation_Craft.repair + occupation_Exec.managerial + occupation_Machine.op.inspct +
#     occupation_Other.service + occupation_Prof.specialty + occupation_Transport.moving +
...
nn_model <- nnet(income ~ ., data = train.nn.reduced, size = 3, maxit = 200)
plotnet(nn_model)
```

``` {r}
nn_probs <- predict(nn_model, newdata = test.nn.reduced, type = "raw")
nn_pred <- ifelse(nn_probs > 0.5, 1, 0)


cm <- confusionMatrix(
  factor(nn_pred, levels = c(0, 1)),
  factor(test.nn.reduced$income, levels = c(0, 1)),
  positive = "1"
)
cm
```
...
``` {r}
set.seed(123)
# Change hidden layer to 5 using rule of thumb sqrt(25) because we have 25 predictors after dummy encoding
nn_model2 <- nnet(income ~ ., data = train.nn.reduced, size = 5, maxit = 200)
plotnet(nn_model2)
```

``` {r}
nn_probs2 <- predict(nn_model2, newdata = test.nn.reduced, type = "raw")
nn_pred2 <- ifelse(nn_probs2 > 0.5, 1, 0)


cm2 <- confusionMatrix(
  factor(nn_pred2, levels = c(0, 1)),
  factor(test.nn.reduced$income, levels = c(0, 1)),
  positive = "1"
)
cm2
```
``` {r}
# Low accuracy from nnet() so lets try using train from caret package with method = "nnet"
grid <- expand.grid(size = c(5, 7, 9, 11), decay = c(0.0001, 0.001, 0.01, 0.05, 0.1, 0.2))

train.nn.reduced$income <- as.factor(train.nn.reduced$income)

nn_model3 <- train(
  income ~ ., data = train.nn.reduced,
  method = "nnet",
  tuneGrid = grid,
...
``` {r}
test.nn.reduced$income <- factor(test.nn.reduced$income, levels = c("0", "1"))

nn_pred4 <- predict(nn_model4, newdata = test.nn.reduced, type = "class")
nn_pred4 <- factor(nn_pred4, levels = c("0", "1"))

cm4 <- confusionMatrix(
  data = nn_pred4,
  reference = test.nn.reduced$income,
  positive = "1"
)
